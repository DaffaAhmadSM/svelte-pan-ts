...
export async function load({ fetch, cookies, params }) {
	...
	const detailTempTimesheet = await fetch(
		import.meta.env.VITE_API_URL + '/timesheet/detail-temp-timesheet/' + params.slug,
		{
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
	...
	const pns = await fetch(
		import.meta.env.VITE_API_URL + '/timesheet/list-pns-temp/' + timesheet.data.id,
		{
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
	...
	const mcd = await fetch(
		import.meta.env.VITE_API_URL + '/timesheet/list-mcd-temp/' + timesheet.data.id,
		{
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
	...
	const diff = await fetch(
		import.meta.env.VITE_API_URL + '/timesheet/list-diff/' + timesheet.data.id,
		{
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
