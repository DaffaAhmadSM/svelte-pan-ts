<script>
	...
	async function resolveConflict(id) {
		...
		const response = await fetch(
			import.meta.env.VITE_API_URL + '/timesheet/resolve-compare-conflict/id/' + id,
			{
				method: 'POST',
				headers: {
	...
	async function reFetchDiff() {
		const diff = await fetch(
			import.meta.env.VITE_API_URL + '/timesheet/list-diff/' + data.timesheet.id,
			{
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
	...
	async function editCompare() {
		...
		const edit = await fetch(
			import.meta.env.VITE_API_URL + '/timesheet/edit-compare-conflict-value/',
			{
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
	...
	async function reFetchDiffDetail(id) {
		const diff = await fetch(
			import.meta.env.VITE_API_URL + '/timesheet/detail-pns-mcd-diff/id/' + id,
			{
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
...
<div class="m-6">
	<div class="flex flex-col gap-6">
		...
		<div class="flex gap-2 text-center">
			...
			<p class="flex gap-2 self-end text-gray-700 opacity-75">
				...
				<svg
					...
					></g><g id="SVGRepo_iconCarrier">
						...
						<path
							...
							fill="#000000"
						>
						</path> <path d="M11.201 5.871h1.6v1.599h-1.6v-1.599z" fill="#000000"> </path>
				...
				<svg
					...
					></g><g id="SVGRepo_iconCarrier">
						<path
							...
							fill="#222222"
						></path> <circle cx="18" cy="6" r="3" fill="#222222"></circle>
					</g></svg
				>
...
<div class="w-full pt-6">
	<Tabs.Root value="mcd" class="rounded-card bg-background-alt shadow-card h-full w-full p-3">
		<Tabs.List
			class="rounded-9px bg-dark-10 shadow-mini-inset dark:bg-background grid h-full w-full grid-cols-3 gap-1 p-1 text-sm leading-[0.01em] font-semibold dark:border dark:border-neutral-600/30"
		>
			<Tabs.Trigger
				value="pns"
				class="data-[state=active]:shadow-mini dark:data-[state=active]:bg-muted h-20 rounded-[7px] bg-transparent py-2 data-[state=active]:bg-slate-100"
				>PNS</Tabs.Trigger
			>
			<Tabs.Trigger
				value="mcd"
		...
		<Tabs.Content value="pns" class="pt-3">
			<UniversalSetupTable
				searchUrl={pnsSearchUrl}
				data={data.dataPns}
				fetchUrl={pnsfetchUrl}
				deleteUrl={pnsdeleteUrl}
				updateUrl={pnsupdateUrl}
				detailUrl={pnsdetailUrl}
				createUrl={pnscreateUrl}
				namePage={pnsnamePage}
				formData={pnsFormData}
				tableList={pnsTableList}
			/>
		</Tabs.Content>
		<Tabs.Content value="mcd" class="pt-3">
			<UniversalSetupTable
				searchUrl={mcdSearchUrl}
				data={data.dataMcd}
				fetchUrl={mcdfetchUrl}
				deleteUrl={mcddeleteUrl}
				updateUrl={mcdupdateUrl}
				detailUrl={mcddetailUrl}
				createUrl={mcdcreateUrl}
				namePage={mcdnamePage}
				formData={pnsFormData}
				tableList={pnsTableList}
			/>
		</Tabs.Content>
						...
						<Accordion>
							{#snippet parent({ handleClick, open })}
								<td class="table-td flex gap-2" onclick={handleClick}>
									<svg
										class="h-5 w-5 text-gray-500 transition {open ? '-rotate-90' : 'rotate-90'}"
										xmlns="http://www.w3.org/2000/svg"
										width="16"
										height="16"
										fill="currentColor"
								...
								<td class="table-td {diff != 0 ? 'hidden' : ''}">
									<button
										class="flex gap-3 bg-green-400 p-2 text-white"
										onclick={() => {
											(alertResolveModal = true), (conflictId = row.id);
										}}
									>
										<svg
											width="20px"
											...
											><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g
												...
												stroke-linejoin="round"
											></g><g id="SVGRepo_iconCarrier">
												<circle cy="12.5" cx="12.5" r="1.75"></circle>
												<circle cy="12.5" cx="3.5" r="1.75"></circle>
												<circle cy="3.5" cx="3.5" r="1.75"></circle>
												<path d="m12.25 7.25v3m-8.5-4.5v4.5"></path>
												<path d="m14.25 1.75-3.5 3.5m0-3.5 3.5 3.5"></path>
											</g></svg
										>
										<p>Resolve</p>
									</button>
													...
													<tr>
														...
														<td class="table-td">
															<button
																...
																onclick={() => {
																	editModal = true;
																	stateEdit.index_id = index;
																	stateEdit.parent_id = row.id;
																	stateEdit.id = pnsrow.id;
																	stateEdit.side = 'pns';
																	formData.kronos_job_number = pnsrow.kronos_job_number;
																	formData.oracle_job_number = pnsrow.oracle_job_number;
																	formData.employee_name = pnsrow.employee_name;
																	formData.parent_id = pnsrow.parent_id;
																	formData.date = pnsrow.date;
																	formData.value = pnsrow.value;
																}}
															>
																<svg
																	width="18px"
														...
														<td class="table-td">
															<button
																...
																onclick={() => {
																	...
																	stateEdit.index_id = index;
																	stateEdit.parent_id = row.id;
																	stateEdit.id = mcdrow.id;
																	stateEdit.side = 'mcd';
																	formData.kronos_job_number = mcdrow.kronos_job_number;
																	formData.oracle_job_number = mcdrow.oracle_job_number;
																	formData.employee_name = mcdrow.employee_name;
																	formData.parent_id = mcdrow.parent_id;
																	formData.date = mcdrow.date;
																	formData.value = mcdrow.value;
																}}
															>
																<svg
																	width="18px"
	...
	<Dialog.Root bind:open={alertResolveModal} closeOnEscape closeOnOutsideClick>
		<Dialog.Portal>
			...
			<Dialog.Content
				class="bg-background fixed top-[50%] left-[50%] z-50 grid max-h-[80%] w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 overflow-scroll border p-6 shadow-lg sm:rounded-lg md:w-full"
			>
				<Dialog.Title class="text-primary-400 m-0 text-lg font-medium">Resolve</Dialog.Title>
