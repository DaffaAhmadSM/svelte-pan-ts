				...
				>
					<summary class="flex flex-row">
						<input
							...
							onchange={(e) => {
								toggleItem(item, /** @type {HTMLInputElement} */ (e.target).checked);
								checkParent();
							}}
							id={'check-' + item.id}
							data-parent={parrentNode}
						/>
						<span class="gap-2 pl-2 text-gray-900">{item.content}</span>
						<svg
					...
					<div class="ml-3 flex flex-row gap-3">
						<div>
							<input
								type="checkbox"
								name="create"
								data-permis-id={item.id}
								onchange={(e) => {
									permisSet(item.id, /** @type {HTMLInputElement} */ (e.target).checked, 'create');
								}}
								checked={createGroup.includes(item.id)}
							/>
							<span>Create</span>
						</div>
						<div>
							<input
								...
								data-permis-id={item.id}
								onchange={(e) => {
									permisSet(item.id, /** @type {HTMLInputElement} */ (e.target).checked, 'update');
								}}
								checked={updateGroup.includes(item.id)}
							/>
							<span>update</span>
						</div>
						<div>
							<input
								...
								data-permis-id={item.id}
								onchange={(e) => {
									permisSet(item.id, /** @type {HTMLInputElement} */ (e.target).checked, 'delete');
								}}
								checked={deleteGroup.includes(item.id)}
							/>
							<span>delete</span>
						</div>
					</div>
					<article class="mx-4">
						<ul
							class="mt-4 flex flex-col gap-2 border-l pl-2"
